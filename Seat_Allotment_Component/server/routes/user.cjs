...
UserRouter.post('/Studentlogin', async (req, res) => {
    try {
        ...
        return res.json({ status: true, message: 'Login Successful', application_number: user.application_number, email: user.email_id, rank: user.rank,name: user.name,surname: user.surname,marks: user.marks,gender: user.gender,password: user.password});
